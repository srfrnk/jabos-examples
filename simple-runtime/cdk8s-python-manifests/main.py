...
class MyChart(Chart):
    def __init__(self, scope: Construct, id: str):
        ...
        k8s.KubeDeployment(scope=self, id='deployment',
                           metadata=k8s.ObjectMeta(name="test-deployment-cdk8s", labels={"app": "test-deployment-cdk8s"}),
                           spec=k8s.DeploymentSpec(
                               replicas=1,
                               selector=k8s.LabelSelector(match_labels={"app": "test-deployment-cdk8s"}),
                               template=k8s.PodTemplateSpec(
                                   metadata=k8s.ObjectMeta(labels={"app": "test-deployment-cdk8s"}),
                                   spec=k8s.PodSpec(
                                       containers=[
                                           k8s.Container(
